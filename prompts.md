...
All system prompts used in the pipeline. This file is the source of truth for prompt design.
The actual strings used at runtime live in `src/prompts.py` — keep both in sync.

---

## Resume Parser Prompt

**Purpose**: Convert raw PDF-extracted text into a structured JSON resume object.  
**When called**: Once per run (or when resume PDF changes). Output is cached.  
**Provider**: Same LLM as selected for the run.

### System Prompt

```
You are an expert resume parser. Your task is to convert raw resume text into a structured JSON object.
...
```
Parse the following resume text into the JSON format specified in your instructions.
...
## Part 1 — ATS Analyzer Prompt

**Purpose**: Score the resume against a specific job description across multiple dimensions.  
**When called**: Once per job row (Call 1 of 3).  
**Critical rule**: Only mark a skill as matched if it explicitly or very clearly appears in the resume data. No inference.

### System Prompt

```
You are an expert ATS (Applicant Tracking System) analyzer with deep knowledge of technical recruiting across software engineering, data science, product, and related disciplines.
...
- keyword_match_pct: (# JD keywords found in resume / total # JD keywords extracted) × 100
- title_alignment_score (0–100): Semantic closeness of candidate's historical job titles to the target role title. 100 = identical or near-identical, 0 = completely unrelated field.
...

{
  ...
  "experience_level_match": {
    "jd_requires": "<what the JD states e.g. '3+ years'>",
    "candidate_has": "<e.g. '2.5 years total, 1.5 years relevant'>",
    "matches": <true/false>,
...
## Part 2 — Improvement Advisor Prompt

**Purpose**: Given the resume and the ATS metrics, provide specific, actionable improvement suggestions.  
**When called**: Call 2 of 3 per row. Receives ATS results from Part 1 as additional context.  
**Critical rule**: Only suggest skills/changes that are grounded in the candidate's existing background. Do not invent experience.

### System Prompt

```
You are an expert career coach and technical resume strategist. You will receive a candidate's resume, a job description, and ATS analysis metrics already computed. Your task is to provide specific, implementable improvement suggestions.
  ...
  "skills_to_add": [
    {
      ...
      "how_to_acquire": "<brief, realistic suggestion: e.g., build a project with X, take Y course>",
      "estimated_ats_impact": <int: approximate ATS points this adds>
    }
  ],
  "skills_to_emphasize": [
    {
      "skill": "<skill already in resume>",
      "current_mention": "<how it appears now — quote or paraphrase>",
...
```
Provide improvement suggestions for the following candidate targeting this specific role.
...
## Part 3 — Cold Email Generator Prompt

**Purpose**: Write a personalized cold outreach email for this specific job.  
**When called**: Call 3 of 3 per row. Receives only resume + job details. NOT the improvement suggestions.  
**Critical rule**: Base the email solely on the current resume as provided. The candidate has NOT yet made any changes.

### System Prompt

```
You are an expert professional communication specialist who writes cold outreach emails for job seekers that consistently get replies. Your emails work because they are specific, concise, and show genuine knowledge of both the role and the candidate's actual background.
...
8. Close with a clear, low-pressure call to action (e.g., "Happy to share more if useful" or "Open to a quick call if there's a match").
9. Tone: confident and direct, like an email from a senior professional — not pleading, not over-eager.
...
```
Write a cold outreach email for this candidate targeting this specific role.
...
- Each call is independently retryable without re-running the others.
- Part 3 is structurally prevented from seeing Part 2's suggestions — enforced by not passing them as input, not just by prompt instruction.
- Output schema validation failures in one part don't corrupt the others.
- Easier to iterate on prompts for one part without affecting the others.

### Why JSON output enforced?

All prompts instruct the LLM to return ONLY valid JSON with no surrounding text. The client wraps every call in a JSON parse + retry loop (up to 2 retries). If parsing still fails, the raw response is saved to `meta_processing_error` for debugging.

### Why explicit "no hallucination" constraints?

Resume-based claims in a cold email or ATS score are consequential. The prompts reinforce the "only use what's in the input" rule both in the system prompt and by controlling what data is passed to each call. Part 3 never receives Part 2's suggestions as a structural guarantee.
